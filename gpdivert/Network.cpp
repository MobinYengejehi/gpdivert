...
std::atomic<uint32_t> g_sessionIdCounter(1);
...
void Network::PacketManager::PacketListener()
{
	...
	while (true)
	{
		...
		{
			L_ERROR << "Packet couldn't be received from WinDivert. [Code: "
				<< GetLastError() << "]" << L_END;
			continue;
		}

		{
			// Parse the packet to retrieve IP/TCP/UDP headers and payload.
			...
			// Our custom header size.
			constexpr size_t extraHeaderSize = sizeof(Network::PacketExtraHeader);
			Network::PacketExtraHeader payloadExtraHeader;
			memset(&payloadExtraHeader, 0, extraHeaderSize);

			if (payloadLength >= extraHeaderSize)
			{
				memcpy(&payloadExtraHeader, payload, extraHeaderSize);
			}

			// If the header is already attached, and it is marked as a server response, process as inbound.
			if (payloadExtraHeader.identifier == GP_SERVER_HEADER)
			{
				goto InboundPacketCapture;
			}
			// If it already is a GP_DIVERT_HEADER packet, then it was already processed.
			else if (payloadExtraHeader.identifier == GP_DIVERT_HEADER)
			{
				goto PacketInject;
			}

			// At this point, if this is an outbound packet, add our custom header.
			if (address.Outbound)
			{
				// (Your code already determines socketHost and socketPort from process info)
				...
				{
					if ((protocol.id == NETWORK_TCP_PROTOCOL_ID && tcpHeader != NULL) ||
				...
				UINT totalHeaderLength = ipHeader->HdrLength * 4 +
					(tcpHeader ? ((tcpHeader->HdrLength >> 4) * 4) : (udpHeader ? sizeof(WINDIVERT_UDPHDR) : 0));

				if (packetLength + extraHeaderSize > NETWORK_MAX_PACKET_SIZE)
					goto PacketInject;

				// Shift payload to make room for our header.
				memmove(packet + totalHeaderLength + extraHeaderSize,
					packet + totalHeaderLength,
					payloadLength);

				Network::PacketExtraHeader extraHeader;
				extraHeader.identifier = Network::GP_DIVERT_HEADER;
				extraHeader.originalIp = ipHeader->DstAddr; // already in network order
				extraHeader.originalPort = htons(destinationPort); // convert to network order
				extraHeader.socketPort = htons(socketPort);
				inet_pton(AF_INET, socketHost.c_str(), &extraHeader.socketHost);

				uint32_t sessId = g_sessionIdCounter.fetch_add(1);
				extraHeader.sessionId = htonl(sessId);
				L_INFO << "[DEBUG] Outbound packet assigned sessionId = " << sessId << L_END;
				...
				// Optionally adjust destination to the proxy’s server.
				...
				L_INFO << "new packet is : " << std::string((const char*)payload, payloadLength) << " | " << payloadLength << L_END;
			}
			else
			{
				// For inbound packets, check if they have our server header.
				if (payloadExtraHeader.identifier != GP_SERVER_HEADER)
				{
					goto PacketInject;
				}
		...
		InboundPacketCapture:
			{
				constexpr size_t extraHeaderSize = sizeof(Network::PacketExtraHeader);
				Network::PacketExtraHeader receivedHeader;
				memcpy(&receivedHeader, payload, extraHeaderSize);
				uint32_t sessId = ntohl(receivedHeader.sessionId);
				L_INFO << "[DEBUG] Inbound packet processed with sessionId = " << sessId << L_END;

				// Remove the header from the payload.
				memmove(payload, payload + extraHeaderSize, payloadLength - extraHeaderSize);
				payloadLength -= extraHeaderSize;
				packetLength -= extraHeaderSize;
				ipHeader->Length = htons(ntohs(ipHeader->Length) - extraHeaderSize);
				if (udpHeader)
					udpHeader->Length = htons(ntohs(udpHeader->Length) - extraHeaderSize);
				// Restore original source address/port from our header.
				inet_pton(AF_INET, ConvertIntegerAddressToString(receivedHeader.originalIp).c_str(), &ipHeader->SrcAddr);
				if (tcpHeader != NULL)
					tcpHeader->SrcPort = receivedHeader.originalPort; // already in network order
				else if (udpHeader != NULL)
					udpHeader->SrcPort = receivedHeader.originalPort;
			}
		}
	...
	while (true)
	{
...

		{
...

			{
				if (tcpHeader != NULL)
				{
					L_INFO << "tcp ignored! : " << std::string((const char*)payload, payloadLength) << " | " << payloadLength << " | " << address.Outbound << L_END;
			...
			if (payloadExtraHeader.identifier == GP_DIVERT_HEADER)
			{
				goto PacketInject;
			}
			else if (payloadExtraHeader.identifier == GP_SERVER_HEADER)
			{
				goto InboundPacketCapture;
			}
		...
		InboundPacketCapture:
			{
				...
				//L_INFO << "server packet detected! : " << std::string((char*)payload, payloadLength) << " | " << payloadLength << L_END;
			}
		}
